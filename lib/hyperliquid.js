...
export function computeSignalScores(analytics) {
  ...
  // 2. TVL Momentum (25%) — requires trailing data
  const tvlScore = analytics.tvlScore ?? 50;

  // 3. Return Momentum (20%) — requires trailing data
  ...
  return { composite, ddScore, tvlScore, momentumScore, volScore, aprScore };
}
...
export function computeTrailingSignals(snapshots, currentNav) {
  ...
  return { tvlScore, momentumScore, volScore };
}
